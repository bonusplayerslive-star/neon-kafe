...
io.on('connection', (socket) => {
    ...
    // SİSTEMİN ÇÖKMESİNİ ENGELLEYEN KRİTİK KONTROLLER
    socket.on('siparis_teslim_edildi', async (id) => {
        if (!id || id === "undefined" || !mongoose.Types.ObjectId.isValid(id)) {
            return console.log("⚠️ Geçersiz Sipariş ID'si engellendi.");
        }
        try {
            const sonuc = await Siparis.findByIdAndUpdate(id, { durum: 'teslim_edildi' });
            if (sonuc) io.emit('siparis_teslim_onayi', id);
        } catch (err) { console.error("Teslimat hatası:", err.message); }
    });
    ...
    socket.on('hesap_kapat', async (masaNo) => {
        ...
        } catch (err) { console.error("Hesap kapatma hatası:", err.message); }
    });

    socket.on('urun_sil', async (id) => {
        if (!id || id === "undefined" || !mongoose.Types.ObjectId.isValid(id)) {
            return console.log("⚠️ Geçersiz Ürün ID'si (silme) engellendi.");
        }
        try {
            await Urun.findByIdAndDelete(id);
        } catch (err) { console.error("Ürün silme hatası:", err.message); }
    });

    socket.on('stok_guncelle', async (data) => {
        if (!data.id || data.id === "undefined" || !mongoose.Types.ObjectId.isValid(data.id)) {
            return console.log("⚠️ Geçersiz Ürün ID'si (stok) engellendi.");
        }
        try {
            await Urun.findByIdAndUpdate(data.id, { stok: data.stok });
        } catch (err) { console.error("Stok güncelleme hatası:", err.message); }
    });
});
